#!/usr/bin/env python3
"""AUQ ROC-AUC Verification on WebShop Benchmark
================================================
...
Arguments:
  ...
  --results_dir         Output directory (default: ./verification_results/webshop/uam)
...
class TestRunner:
    ...
    def __init__(
        ...
        provider: str | None = None,
        include_obs_in_history: bool = True,
        results_dir: str = "./verification_results/webshop/uam",
        n_tasks: int = 100,
        max_steps: int = 25,
    ...
    def run_agent(self) -> tuple[list[list[float]], list[int], dict[str, Any]]:
        ...
        """
        logger.info(f"Running AUQ agent on WebShop: model={self.model_name}")
...
def main():
    parser = argparse.ArgumentParser(description="AUQ verification on WebShop")
    parser.add_argument(
    ...
    parser.add_argument(
        "--results_dir",
        default="./verification_results/webshop/uam",
    ...
    logger.info("=" * 60)
    logger.info("AUQ Verification on WebShop")
    logger.info(
