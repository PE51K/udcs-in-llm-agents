...
"""ReAct baseline hackable agent.

Like the AUQ hackable agent, outputs <think>/<action>/<confidence>/<explanation>,
but confidence and explanation are NOT included in history.
History only contains think + action per step (no UAM memory).
...
class ReActHackableAgent(Agent):
    """ReAct baseline hackable agent.
    ...
    def _build_user_messages(self, obs: dict) -> list[dict]:
        ...
        7. Confidence Elicitation Suffix (for evaluation only)
        """
        msgs = []
