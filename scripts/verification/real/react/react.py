#!/usr/bin/env python3
"""ReAct Baseline ROC-AUC Verification on REAL Benchmark
===========================================
...
Run from project root:
    ...
    uv run --group scripts python -m \
        scripts.verification.real.react.react \
...
Arguments:
  ...
  --results_dir         Output directory (default: ./verification_results/real/auq)
  --log_level           Logging level: DEBUG|INFO|WARNING|ERROR|CRITICAL (default: INFO)
  --random_seed             Random seed for reproducibility (default: None)
  --shuffle_tasks           Randomly shuffle tasks before selection (default: True)
  --include_obs_in_history  Include observation summaries in history (default: False)
  --no-include_obs_in_history  Disable observations in history
...
from .hackable import ReActHackableAgent
...
class ReActAgentArgs(AbstractAgentArgs):
    ...
    def make_agent(self) -> ReActHackableAgent:
        return ReActHackableAgent(
...
class TestRunner:
    ...
    def run_agent(self) -> dict[str, dict[str, Any]]:
        ...
        logger.info(f"Running ReAct agent: model={self.model_name}")

        agent_args = ReActAgentArgs(
    ...
    def extract_episode_data(
        ...
        """Extract per-step confidences and success labels from results.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ReAct baseline on REAL")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--results_dir",
        default=None,
        help="Output directory (default: ./verification_results/real/react/{model})",
    )
    parser.add_argument(
        "--log_level",
        default="INFO",
    ...
    if args.results_dir is None:
        ...
        args.results_dir = f"./verification_results/real/react/{model_dir}"
    ...
    logger.info("=" * 60)
    logger.info("ReAct Baseline on REAL")
    logger.info(
    ...
    all_step_confidences, labels, task_details = runner.extract_episode_data(results)
