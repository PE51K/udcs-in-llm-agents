...
"""Proposed v4 hackable agent for REAL benchmark.
...
Memory structure (paper Eq. 3):
    M_t = {(o_i, a_i, ĉ_i, ê_i, û_i, ûe_i)}_{i=0}^{t-1}
...
class ProposedV4HackableAgent(Agent):
    """Proposed v4 hackable agent: UAM confidence + u_request decomposition.

    Extends AUQ (Huang et al.) with a separate request uncertainty (u_request) source.
    - Confidence Elicitation: <think>/<u_request>/<u_request_explanation>/<action>/<confidence>/<explanation>
    - UAM with both confidence and u_request propagated in history
    """
    ...
    def _build_system_messages(self) -> list[dict]:
        """System message with <think>/<u_request>/<action> format instruction."""
        if self.chat_mode:
            base = (
                "# Instructions\n\n"
        ...
        base += (
            ...
            "After thinking, assess request uncertainty in <u_request>...</u_request> tags "
            "and explain in <u_request_explanation>...</u_request_explanation> tags.\n\n"
            ...
            "After your action, provide confidence in <confidence>...</confidence> tags "
            "and an explanation in <explanation>...</explanation> tags."
        )
        return [{"type": "text", "text": base}]

    def _build_user_messages(self, obs: dict) -> list[dict]:
        """Build user messages with UAM format and elicitation suffix.

        Structure:
        1. Goal / Chat messages
        2. Current observation (AXTree, DOM, Screenshot)
        3. Action Space
        4. UAM History (Variant B: each step has obs?, think, u_request, action, confidence, explanation)
        5. Error from last action (if any)
        6. Next action instruction (with step count)
        7. u_request assessment + Confidence Elicitation Suffix
        """
        msgs = []
        ...
        # --- u_request assessment + Confidence Elicitation Suffix ---
        msgs.append(
            {
                "type": "text",
                "text": (
    ...
    def get_action(self, obs: dict) -> tuple[str, dict]:
        ...
        Paper Algorithm 1, Phase 3 (Memory Update):
            M_{t+1} ← M_t ∪ {(o_t, a_t, ĉ_t, ê_t, û_t, ûe_t)}

        Returns:
            action: Parsed action string
            agent_info: Dict with step_confidence, step_u_request, step_explanation, etc.
        """
