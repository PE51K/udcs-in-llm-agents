...
Run from project root:
    ...
    uv run --group scripts python -m \
        scripts.verification.real.uam.uam \
...
Arguments:
  ...
  --results_dir         Output directory (default: ./verification_results/real/uam)
...
from .hackable import AUQHackableAgent

logger = logging.getLogger(__name__)


@dataclasses.dataclass
class AUQAgentArgs(AbstractAgentArgs):
    """Agent args for AUQ uncertainty estimation."""

    agent_name: str = "AUQAgent"
    ...
    def make_agent(self) -> AUQHackableAgent:
        return AUQHackableAgent(
...
class TestRunner:
    ...
    def run_agent(self) -> dict[str, dict[str, Any]]:
        ...
        logger.info(f"Running AUQ agent: model={self.model_name}")

        agent_args = AUQAgentArgs(
            agent_name="AUQAgent",
            model_name=self.model_name,
            provider=self.provider,
            use_screenshot=self.use_screenshot,
...
def main():
    ...
    parser.add_argument(
        "--results_dir",
        default=None,
        help="Output directory (default: ./verification_results/real/uam/{model})",
    )
    parser.add_argument(
        "--log_level",
        default="INFO",
    ...
    if args.results_dir is None:
        ...
        args.results_dir = f"./verification_results/real/uam/{model_dir}"
