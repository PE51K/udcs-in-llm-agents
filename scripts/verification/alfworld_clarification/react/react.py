#!/usr/bin/env python3
"""ReAct Clarification Benchmark on ALFWorld
================================================

Clarification-seeking variant of the ReAct baseline. 50% of tasks are
underspecified (object or receptacle dropped from goal). The agent has an
request_clarification action to request a more specified goal. If the agent
requests clarification on an underspecified task, the full goal is revealed and
the episode continues.
Evaluates clarification precision/recall/F1/accuracy alongside standard
...
Run from project root:
    ...
    uv run --group scripts python -m \
        scripts.verification.alfworld_clarification.react.react \
...
def _read_traj_data(game_file_path: str) -> dict:
    """Read traj_data.json from same directory as game file."""
...
def _make_underspecified_alfworld(traj_data: dict, drop_object: bool) -> str:
    """Create underspecified goal by dropping object or receptacle."""
...
def _build_clarification_task_set(n_tasks: int, random_seed: int | None = None) -> list[tuple[int, bool]]:
    """Generate (label, drop_object) pairs. 50/50 specified/underspecified."""
...
class ReActAlfWorldAgent:
    """ReAct baseline agent for ALfWorld text environment.
    ...
    def _build_user_messages(
        ...
        History contains only think + action per step (no confidence/explanation).
        Confidence is still elicited for evaluation purposes.
        """
        parts = []
        ...
        # --- Step count + History (no confidence/explanation) ---
    ...
    def get_action(
        ...
        """Get action and update history (think+action only, no confidence/explanation).
...
class TestRunner:
    """Runs ReAct agent on ALfWorld clarification benchmark."""
    ...
    def __init__(
        ...
        provider: str | None = None,
        include_obs_in_history: bool = True,
        results_dir: str = "./verification_results/alfworld_clarification/react",
        n_tasks: int = 100,
        max_steps: int = 25,
    ...
    def run_agent(self) -> tuple[list[list[float]], list[int], list[int], list[int], dict[str, Any]]:
        ...
        """
        logger.info(f"Running ReAct clarification agent on ALfWorld: model={self.model_name}")
        ...
        for game_idx, game_file in enumerate(alfred_env.game_files):
            ...
            logger.info(
                f"--- Game {game_idx + 1}/{num_games}: {Path(game_file).parent.name} "
            ...
            try:
                ...
                if task_label == 1:
                    try:
                        ...
                        logger.warning(f"  Could not underspecify: {e}, using original")
                        goal = original_goal
                        task_label = 0
                ...
                for step in range(self.max_steps):
                    ...
                    logger.debug(
                        f"  step={step}, action={action!r}, done={done}, won={won}"
                    )
                ...
                logger.info(
                    ...
                    f"success={success}, asked={agent_asked}"
                )
...
def main():
    parser = argparse.ArgumentParser(description="ReAct clarification benchmark on ALfWorld")
    ...
    parser.add_argument("--results_dir", default="./verification_results/alfworld_clarification/react")
    ...
    logger.info("=" * 60)
    logger.info("ReAct Clarification Benchmark on ALfWorld")
