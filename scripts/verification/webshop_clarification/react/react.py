#!/usr/bin/env python3
"""ReAct Clarification Benchmark on WebShop
================================================
...
Run from project root:
    ...
    uv run --group scripts python -m \
        scripts.verification.webshop_clarification.react.react \
...
class TestRunner:
    """Runs ReAct agent on WebShop clarification benchmark."""
    ...
    def __init__(
        ...
        provider: str | None = None,
        include_obs_in_history: bool = True,
        results_dir: str = "./verification_results/webshop_clarification/react",
        n_tasks: int = 100,
        max_steps: int = 25,
    ...
    def run_agent(self) -> tuple[list[list[float]], list[int], list[int], list[int], dict[str, Any]]:
        ...
        Returns:
            ...
            task_labels: 0=specified, 1=underspecified per task
            all_agent_asked: 1 if agent used request_clarification, 0 otherwise
            task_details: Per-session detail dict
        """
        logger.info(f"Running ReAct clarification agent on WebShop: model={self.model_name}")
        ...
        # Build clarification task set (50% specified, 50% underspecified)
        ...
        for task_idx, session_idx in enumerate(task_sessions):
            ...
            try:
                # Get the original fully specified goal
                obs, _ = env.reset(session=session_idx)
                original_goal = env.instruction_text

                # For underspecified tasks, strip attributes/options
                if task_label == 1:
                ...
                for step in range(self.max_steps):
                    ...
                    # Detect request_clarification action
                    if action.lower().startswith("request_clarification"):
                        agent_asked = 1
                        logger.info(f"  Step {step}: agent requested clarification")
                        # If underspecified, reveal the full goal
                        if task_label == 1:
                            goal = original_goal
                            logger.info(f"  Revealed full goal: {original_goal[:100]}...")
                        # Don't call env.step — request_clarification is not a real action
                        # Continue the episode with updated (or same) goal
                        continue
    ...
    def compute_clarification_metrics(
        ...
        """Compute precision/recall/F1/accuracy for clarification decisions.

        TP = underspecified AND agent asked
        FP = specified AND agent asked
        FN = underspecified AND agent did NOT ask
        TN = specified AND agent did NOT ask
        """
...
def main():
    parser = argparse.ArgumentParser(description="ReAct clarification benchmark on WebShop")
    parser.add_argument(
    ...
    parser.add_argument(
        "--results_dir",
        default="./verification_results/webshop_clarification/react",
    ...
    logger.info("=" * 60)
    logger.info("ReAct Clarification Benchmark on WebShop")
    logger.info(
    ...
    for method, m in metrics.items():
        logger.info(
            f"  {method}: AUROC={m['auroc']:.4f}  ECE={m['ece']:.4f}  Brier={m['brier']:.4f}"
        )
    ...
    # Annotate task_details with trajectory-level confidences
