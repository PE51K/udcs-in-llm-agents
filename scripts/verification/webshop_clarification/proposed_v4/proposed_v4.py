#!/usr/bin/env python3
"""Proposed v4 Clarification Benchmark on WebShop
================================================

Extends AUQ (Huang et al.) with a separate request uncertainty (u_request) source.
50% of tasks are underspecified (attributes/options stripped from instruction).
The agent has a request_clarification action to request a more specified goal. If the
...
alongside standard fault-detection metrics (confidence and u_request).

Run from project root:
    export OPENAI_API_KEY=$OPENAI_API_KEY
    uv run --group scripts python -m \
        scripts.verification.webshop_clarification.proposed_v4.proposed_v4 \
...
class ProposedV4WebShopAgent:
    ...
    def _build_user_messages(
        ...
        3. Available actions
        4. UAM History (Variant B: obs, think, u_request, action, confidence, explanation)
...
class TestRunner:
    """Runs Proposed v4 agent on WebShop clarification benchmark."""
    ...
    def __init__(
        ...
        provider: str | None = None,
        include_obs_in_history: bool = True,
        results_dir: str = "./verification_results/webshop_clarification/proposed_v4",
        n_tasks: int = 100,
        max_steps: int = 25,
    ...
    def run_agent(self) -> tuple[list[list[float]], list[list[float]], list[int], list[int], list[int], dict[str, Any]]:
        ...
        logger.info(f"Running proposed v4 clarification agent on WebShop: model={self.model_name}")
...
def main():
    parser = argparse.ArgumentParser(description="Proposed v4 clarification benchmark on WebShop")
    ...
    parser.add_argument("--results_dir", default="./verification_results/webshop_clarification/proposed_v4")
    ...
    logger.info("=" * 60)
    logger.info("Proposed v4 Clarification Benchmark on WebShop")
