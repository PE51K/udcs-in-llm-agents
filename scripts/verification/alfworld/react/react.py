#!/usr/bin/env python3
"""ReAct Baseline ROC-AUC Verification on ALfWorld Benchmark
==================================================
...
Arguments:
  ...
  --results_dir         Output directory (default: ./verification_results/alfworld/react))
...
class TestRunner:
    ...
    def __init__(
        ...
        provider: str | None = None,
        include_obs_in_history: bool = True,
        results_dir: str = "./verification_results/alfworld/react",
        n_tasks: int = 100,
        max_steps: int = 25,
    ...
    def run_agent(self) -> tuple[list[list[float]], list[int], dict[str, Any]]:
        ...
        """
        logger.info(f"Running ReAct agent on ALfWorld: model={self.model_name}")
...
def main():
    parser = argparse.ArgumentParser(description="ReAct baseline on ALfWorld")
    parser.add_argument(
    ...
    parser.add_argument(
        "--results_dir",
        default="./verification_results/alfworld/react",
    ...
    logger.info("=" * 60)
    logger.info("ReAct Baseline on ALfWorld")
    logger.info(
